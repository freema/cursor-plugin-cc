...
var parser = new YargsParser({
  ...
  require: (path6) => {
    if (typeof __require !== "undefined") {
      ...
      return JSON.parse(readFileSync3(path6, "utf8"));
    } else {
...
async function authStatus() {
  try {
    ...
    const res = await execa(bin, ["status"], { reject: false, timeout: 5e3 });
    const text = `${res.stdout ?? ""}
${res.stderr ?? ""}`.toLowerCase();
    const loggedIn = res.exitCode === 0 && (text.includes("logged in") || text.includes("authenticated") || text.includes("signed in"));
...
async function listModels() {
  try {
    const bin = await resolveBin();
    const res = await execa(bin, ["--list-models"], { reject: false, timeout: 1e4 });
    if (res.exitCode !== 0) {
      const fallback = await execa(bin, ["models"], { reject: false, timeout: 1e4 });
...
function repoHash(repoRoot) {
  const canonical = existsSync(repoRoot) ? realpathSync(repoRoot) : resolve2(repoRoot);
...
function checkBuild() {
  const dist = join2(pluginRoot(), "dist");
  if (!existsSync2(dist)) {
    return {
      ok: false,
  ...
  const entry = join2(dist, "setup.js");
...
async function doctor() {
  ...
  if (bin) {
    try {
      const ver = await execa(bin, ["--version"], { reject: false, timeout: 5e3 });
      checks.push([
...
async function baseCheck() {
  ...
  try {
    ...
    const auth = await authStatus();
    lines.push(
      auth.loggedIn ? "- \u2713 Cursor CLI is logged in." : "- \u2717 Cursor CLI is not logged in. Run `cursor-agent login` in a terminal."
    );
    const build = checkBuild();
