...
async function listConfiguredMcps() {
  try {
    const bin = await resolveBin();
    const res = await execa(bin, ["mcp", "list"], { reject: false, timeout: 5e3 });
...
// scripts/browser.ts
var BOOLEAN_FLAGS = ["background", "fresh", "force", "git-check", "skip-mcp-check", "help"];
var MCP_NAME = "chrome-devtools";
function buildBrowserPrompt(url, what) {
  const urlLine = url ? `**Target URL:** ${url}` : `**Target URL:** not supplied \u2014 you MUST discover it before testing. In order:
...
function usedBrowserMcp(events) {
  for (const ev of events) {
    for (const tu of extractToolUses(ev)) {
      ...
      if (name.includes("chrome") || name.includes("devtools") || name.startsWith("mcp_") || name.includes("take_snapshot") || name.includes("navigate_page") || name.includes("list_pages") || name.includes("new_page") || name.includes("take_screenshot") || name.includes("list_console_messages") || name.includes("list_network_requests") || name.includes("evaluate_script")) {
        return true;
      }
    }
  }
...
function looksLikeUrl(token) {
  return /^https?:\/\//i.test(token) || /^localhost(:\d+)?(\/|$)/i.test(token) || /^\/\//.test(token);
}
...
function parseFlags(argv) {
  ...
  const explicitForce = "force" in flags ? Boolean(flags["force"]) : void 0;
  const force = explicitForce === void 0 ? true : explicitForce;
  const noGitCheck = flags["gitCheck"] === false || flags["git-check"] === false || flags["no-git-check"] === true || flags["noGitCheck"] === true;
  const skipMcpCheck = flags["skip-mcp-check"] === true || flags["skipMcpCheck"] === true || flags["mcpCheck"] === false;
...
async function preflightMcp() {
  ...
  if (!match) {
    return {
      ok: false,
      detail: `\`${MCP_NAME}\` MCP is not configured. Configured MCPs: ${mcps.map((m) => m.name).join(
        ", "
...
`);
  ...
  const eventsAsRec = result.events;
...
A curl/wget/fetch check is not a browser test. Re-run after fixing the MCP.`
  ...
  updateJob(root, jobId, {
    ...
    summary: warnings.length > 0 ? `${summary.summary}
