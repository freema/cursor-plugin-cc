...
// scripts/resume.ts
async function main2(rawArgv) {
  const argv = rawArgv.slice();
  const hasResume = argv.some((a2) => a2 === "--resume" || a2.startsWith("--resume="));
  if (!hasResume) argv.unshift("--resume");
  return main(argv);
}
var invokedAsScript2 = (() => {
  try {
...
if (invokedAsScript2) {
  main2(process.argv.slice(2)).then((code) => process.exit(code)).catch((err) => {
