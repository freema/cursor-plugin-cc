...
function listJobs(repoPath, opts = {}) {
  ...
  for (const f of files) {
    try {
      const raw = readFileSync4(join2(dir, f), "utf8");
      records.push(JobRecord.parse(JSON.parse(raw)));
    } catch {
      continue;
    }
  }
  records.sort((a2, b) => a2.startedAt < b.startedAt ? 1 : -1);
...
async function cancelJob(repoPath, id, graceMs = 5e3) {
...
function findRunningJobs(repoPath) {
